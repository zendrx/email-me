...
class Alias
  ...
  def self.create(user_id : Int32, local_part : String, domain : String, forward_to : String, is_paid_user : Bool = false) : Tuple(Bool, String | Int32)
    ...
    begin
      ...
      alias_id = EmailMe::DB.connection.query_one(
        "SELECT currval('aliases_id_seq')",
        as: Int32
      )
  ...
  def self.delete(alias_id : Int32, user_id : Int32, is_admin : Bool = false) : Bool
    ...
    if alias_info.domain != DEFAULT_DOMAIN
      domain_owner = EmailMe::DB.connection.query_one?(
    ...
    if is_admin
      result = EmailMe::DB.connection.exec(
  ...
  def self.find_by_user(user_id : Int32, active_only : Bool = true) : Array(Alias)
    ...
    results = EmailMe::DB.connection.query_all(
      query, user_id, as: {Int32, Int32, String, String, String, Bool, Time}
    )
  ...
  def self.get_domains_for_user(user_id : Int32) : Array(String)
    ...
    # Team domains
    team_domains = EmailMe::DB.connection.query_all(
      "SELECT d.domain
       FROM domains d
